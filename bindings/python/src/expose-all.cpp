...
#include "expose-gqp-cones.hpp"
#include "expose-gqp.hpp"
...
template <typename T> void exposeGQPAlgorithms(nanobind::module_ m) {
  proxsuite::proxgqp::python::exposeCones<T>(m);
  proxsuite::proxgqp::python::exposeGQP<T>(m);
}

template <typename T> void exposeCommon(nanobind::module_ m) {
  exposeResults<T>(m);
  exposeSettings<T>(m);
...
void exposeSparseAlgorithms(nanobind::module_ m) {
...
template <typename T> void exposeDenseAlgorithms(nanobind::module_ m) {
...
template <typename T> void exposeBackward(nanobind::module_ m) {
  dense::python::backward<T>(m);
}

#ifdef PROXSUITE_PYTHON_INTERFACE_WITH_OPENMP
template <typename T> void exposeDenseParallel(nanobind::module_ m) {
  dense::python::solveDenseQpParallel<T>(m);
}
template <typename T, typename I>
void exposeSparseParallel(nanobind::module_ m) {
...
NB_MODULE(PYTHON_MODULE_NAME, m) {
  ...
  nanobind::module_ proxgqp_module =
      m.def_submodule("proxgqp", "The proxGQP solver of the proxSuite library");
  exposeGQPAlgorithms<f64>(proxgqp_module);
  ...
  helpers_module.def("printVersion", helpers::printVersion,
                     nanobind::arg("delimiter") = ".",
                     "Print the current version of the package.");
  helpers_module.def("checkVersionAtLeast", helpers::checkVersionAtLeast,
