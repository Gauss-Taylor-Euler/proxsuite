...
namespace dense {
namespace python {
template <typename T> void exposeDenseModel(nanobind::module_ m) {

  ::nanobind::class_<BackwardData<T>>(m, "BackwardData")
      .def(::nanobind::init(), "Default constructor.")
      .def("initialize", &BackwardData<T>::initialize, nanobind::arg("n") = 0,
           nanobind::arg("n_eq") = 0, nanobind::arg("n_in") = 0,
           "Initialize the jacobians (allocate memory if not already done) and "
           "set"
  ...
  ::nanobind::class_<proxsuite::proxqp::dense::Model<T>>(m, "model")
      .def(::nanobind::init<i64, i64, i64>(), nanobind::arg("n") = 0,
           nanobind::arg("n_eq") = 0, nanobind::arg("n_in") = 0,
...
namespace sparse {
namespace python {
template <typename T, typename I> void exposeSparseModel(nanobind::module_ m) {
